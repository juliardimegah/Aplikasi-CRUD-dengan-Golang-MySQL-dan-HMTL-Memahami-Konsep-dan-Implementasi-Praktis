...
5. Ekstrak paket instalasi yang telah Anda unduh dengan perintah berikut (gantikan `nama_file.tar.gz` dengan nama file yang sesuai):
 

3. Setelah ekstraksi selesai, atur variabel lingkungan `GOROOT` dengan perintah berikut:
 

4. Tambahkan direktori `bin` Go ke variabel lingkungan `PATH`:
 

5. Agar perubahan ini berlaku secara permanen, tambahkan perintah yang sama ke berkas profil shell yang sesuai (misalnya, `~/.bashrc` atau `~/.bash_profile`).
6. Verifikasi instalasi Go dengan menjalankan perintah `go version`. Jika versi Go muncul, instalasi telah berhasil.
...
4. Ekstrak berkas XAMPP dengan perintah berikut: `tar xvfz xampp-linux-x64-<versi>.tar.gz`. Gantilah `<versi>` dengan versi yang tepat sesuai dengan berkas yang Anda unduh.
5. Setelah ekstraksi selesai, pindah ke direktori XAMPP yang baru dibuat dengan perintah `cd xampp`.
6. Jalankan installer XAMPP dengan perintah `sudo ./xampp-linux-x64-<versi>-installer.run`. Gantilah `<versi>` dengan versi yang sesuai dengan berkas yang Anda unduh.
7. Ikuti petunjuk di layar untuk menyelesaikan proses instalasi XAMPP. Anda mungkin perlu memasukkan kata sandi superuser (root) Linux Anda.
8. Setelah instalasi selesai, buka terminal dan jalankan XAMPP Control Panel dengan perintah `sudo /opt/lampp/manager-linux-x64.run`.
9. Di XAMPP Control Panel, aktifkan modul MySQL dengan mengklik kotak centang di sebelah "MySQL". Ini akan menjalankan server MySQL.
10. Setelah server MySQL berjalan, Anda dapat membuka phpMyAdmin di browser untuk mengelola basis data. Buka browser dan kunjungi URL http://localhost/phpmyadmin/.
11. Di phpMyAdmin, buat basis data baru untuk aplikasi CRUD Anda. Klik tombol "New" di bagian kiri, masukkan nama basis data yang diinginkan, dan pilih kolasi (collation) yang sesuai. Misalnya, Anda dapat memberi nama basis data "aplikasi_crud" dan pilih kolasi "utf8_general_ci".
12. Setelah membuat basis data, Anda dapat membuat tabel dan mengelola skema basis data sesuai kebutuhan aplikasi CRUD Anda menggunakan antarmuka phpMyAdmin yang intuitif.
13. Pastikan Anda mencatat nama basis data, nama tabel, dan struktur kolom yang akan digunakan dalam aplikasi CRUD Anda. Informasi ini akan diperlukan dalam pengembangan selanjutnya.
